...
const ApplicationsTab: React.FC<ApplicationsTabProps> = ({ onViewApplication }) => {
  ...
  const filteredApplications = logisticsApplications.filter((app: LogisticsApplication) => {
    const matchesSearch = 
      (app.companyName?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false) ||
      (app.contactPerson?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {filteredApplications.map((application: LogisticsApplication) => (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {application.requestedServices?.map((s: string) => s.replace('_', ' ')).join(', ') ?? ''}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-500">Zones</p>
                      <p className="font-medium">
                        {application.proposedZones?.length ?? 0} zone{application.proposedZones?.length !== 1 ? 's' : ''}
                      </p>
                    </div>
                    <div>
