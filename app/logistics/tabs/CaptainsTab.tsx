...
const CaptainsTab: React.FC = () => {
  ...
  const filteredCaptains = captains.filter((captain: Captain) => {
    const matchesSearch = captain.name.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleToggleStatus = (captainId: string) => {
    setCaptains(prev => prev.map((captain: Captain) => {
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {captains.filter((c: Captain) => c.status === 'active').length}
                </p>
              </div>
